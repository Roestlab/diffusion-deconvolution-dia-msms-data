...
@click.option(
    "--warmup-epochs", default=5, help="Number of warmup epochs for learning rate scheduler"
)
...
@click.option(
    ...
    help="Model class to use. (CustomTransformer, Unet1D)",
)
# Data settings
...
@click.option(
    "--wandb-name",
    default=None,
    help="Weigths & Biases name. If None, Wandb will generate a random name",
)
@click.option("--wandb-id", default=None, help="Weigths & Biases run ID")
@click.option(
    "--wandb-resume",
    default=None,
    help="Weigths & Biases resume ID if run crashed or stopped early. Allowed values are None, allow, must, never",
)
@click.option(
    "--wandb-architecture",
    default="DDIM(CustomTransformer)",
    help="Weigths & Biases model architecture name",
)
@click.option("--wandb-dataset", default="Josh_GPF_DIA", help="Weigths & Biases dataset name")
@click.option(
    "--wandb-mode",
    default="offline",
    help="Weigths & Biases mode. (offline, online). Default is offline, which means metrics are only saved locally. You will need to run wandb sync to upload the metrics to the cloud.",
)
def train(
    epochs,
    warmup_epochs,
    ...
    diffusion_model.train(
        data_loader, batch_size, epochs, warmup_epochs, learning_rate, use_wandb, checkpoint_path
    )
